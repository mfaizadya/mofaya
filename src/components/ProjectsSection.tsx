      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#141518] border border-[#4A4A4A] mb-3">
            <Terminal size={14} className="text-[#6D8196]" />
            <span className="text-xs font-mono text-[#CBCBCB]">FEATURED WORK &amp; CAPSTONE</span>
          </div>
          <ScrollReveal
            as="h2"
            baseOpacity={0.05}
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-mono font-medium transition-all ${
                    isActive
                      ? 'bg-[#6D8196] text-[#FFFFE3] shadow-md shadow-[#6D8196]/20'
                      : 'bg-[#141518] border border-[#4A4A4A] text-[#CBCBCB] hover:text-[#FFFFE3] hover:border-[#6D8196]'
                  }`}
                >
                  {cat}
                </button>
      ...
      <div className="w-full max-w-5xl mx-auto" style={{ height: '88vh', minHeight: '760px' }}>
        ...
        >
          {filteredProjects.map((project) => (
            <ScrollStackItem
              key={project.id}
              itemClassName="group bg-[#141518] border border-[#4A4A4A]/70 hover:border-[#6D8196] overflow-hidden cursor-pointer transition-colors duration-300 shadow-xl hover:shadow-2xl"
            >
              <div onClick={() => setSelectedProject(project)}>
                ...
                <div className="px-6 py-5 sm:px-8 sm:py-6">
                  ...
                  <div className="flex items-center justify-between gap-2 mb-3">
                    <span className="px-3 py-1.5 rounded-md bg-[#1D1E22] text-[#6D8196] text-xs font-mono font-semibold border border-[#4A4A4A]/50">
                      {project.category}
                    </span>
                    <span className="text-xs font-mono text-[#CBCBCB]">
                      {project.period}
                    </span>
                  </div>

                  {/* Title & Role */}
                  <h3 className="text-2xl sm:text-3xl font-extrabold text-[#FFFFE3] group-hover:text-[#6D8196] transition-colors mb-1">
                    {project.title}
                  </h3>
                  ...
                  {/* Summary */}
                  <p className="text-sm text-[#CBCBCB] leading-relaxed mb-5 line-clamp-2">
                    {project.summary}
                  </p>
                    ...
                    <div className="flex items-center gap-1.5">
                      {project.metrics && (
                        <span className="flex items-center gap-1 text-[#FFFFE3]">
                          <GitCommit size={14} className="text-[#6D8196]" />
                          {project.metrics}
                        </span>
                      )}
                    </div>
                    <span className="text-[#6D8196] group-hover:underline flex items-center gap-1 font-semibold">
          ...
          >
            ...
            <div className="p-6 sm:p-8">
              ...
              <div className="flex items-start justify-between gap-4 mb-6">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="px-2.5 py-1 rounded-md bg-[#1D1E22] text-[#6D8196] text-xs font-mono font-semibold">
                  ...
                  <h3 className="text-2xl sm:text-3xl font-extrabold text-[#FFFFE3]">
                    {selectedProject.title}
                  </h3>
                  <p className="text-sm font-mono text-[#6D8196] mt-1">
              ...
              <div className="mb-6 p-4 rounded-xl bg-[#0D0E10] border border-[#4A4A4A]/50">
                <p className="text-sm sm:text-base text-[#CBCBCB] leading-relaxed">
              ...
              {/* Contributions list */}
              <div className="mb-6">
                <h4 className="text-xs font-mono uppercase tracking-wider text-[#FFFFE3] mb-3 flex items-center gap-1.5">
                  <Layers size={15} className="text-[#6D8196]" />
              ...
              <div className="mb-8">
                <h4 className="text-xs font-mono uppercase tracking-wider text-[#FFFFE3] mb-3">
                  Tech Stack &amp; Tools
                </h4>
                <div className="flex flex-wrap gap-2">
              ...
              <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-[#4A4A4A]">
                ...
                {selectedProject.liveUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#6D8196] text-[#FFFFE3] font-mono font-bold text-xs hover:bg-[#6D8196]/90 transition-all"
                  >
